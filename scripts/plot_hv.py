"""Plot hypervolume learning curves with standard-error bands."""
...
def draw(ax: Axes, csv_path: str | Path = Path(__file__).with_name("hvroom.csv")) -> None:
    data = pd.read_csv(csv_path)
    values = _value_columns(data)
    if not values:
        raise ValueError("Expected at least one hypervolume value column.")
    ...
    ax.set_title("(c) Normalized Hypervolume")
    ax.legend(
    frameon=False,
    fontsize=5,          # 字体大小，调小避免遮挡
    ncol=2,              # 图例列数，可改成 1 或 3
    loc="lower right",   # 位置
    labelspacing=0.3,    # 每行 label 之间的竖向间距
    columnspacing=0.8,   # 多列之间的横向间距
    handlelength=1,    # 线段长度
    handletextpad=0.4,   # 线段和文字之间的距离
    borderaxespad=0.1,   # 图例和坐标轴边缘距离
    )
...
def main() -> None:
    ...
    parser.add_argument("--csv", type=Path, default=Path(__file__).with_name("hvroom.csv"))
    parser.add_argument("--output", type=Path, default=Path("outputs/hvroom.pdf"))
