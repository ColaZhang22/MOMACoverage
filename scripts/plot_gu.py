"""Plot global-utility learning curves with standard-error bands."""
...
def draw(ax: Axes, csv_path: str | Path = Path(__file__).with_name("gu.csv")) -> None:
    data = pd.read_csv(csv_path)
    values = _value_columns(data)
    if not values:
        raise ValueError("Expected at least one global-utility value column.")
    ...
    ax.set_title("(a) Global Expected Utility")
    ax.legend(
        frameon=False,
        fontsize=5,
        ncol=2,
...
def main() -> None:
    ...
    parser.add_argument("--csv", type=Path, default=Path(__file__).with_name("guroom.csv"))
    parser.add_argument("--output", type=Path, default=Path("outputs/guroom.pdf"))
