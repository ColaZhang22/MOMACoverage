...
from benchmarl.sampler.adaptive_preference_sampler import SharedPreferenceState

_UNITY_ENV_NAMES = frozenset({"sar", "fheragent"})
...
class ExperimentConfig:
    ...
    parallel_evaluation: bool = MISSING
    evaluation_parallel_rollout_rounds: int = MISSING
    ...
    adaptive_preference_sampler: bool = MISSING
...
class Experiment(CallbackNotifier):
    ...
    def _setup(self):
        ...
        if self.config.adaptive_preference_sampler == True:
            self._mp_manager = multiprocessing.Manager()  # 必须在 ParallelEnv 之前
            noi = self.config.evaluation_episodes * self.config.evaluation_parallel_rollout_rounds
            self.anchor_points = np.array([[8.5, -4.5], [95.0, -200.0]])
            self.adaptive_preference_sampler = SharedPreferenceState(manager=self._mp_manager, noi=noi, anchor_points=self.anchor_points)
            print("Using adaptive preference sampler")
        else:
            self.adaptive_preference_sampler = None
            print("Not using adaptive preference sampler")
    ...
    def _setup_task(self):
        print("--------------------------------SETUP TASK--------------------------------")
        # Initialize test env
        test_env_fun = self.task.get_env_fun(
            worker_id=0,
            num_envs=self.config.evaluation_episodes,
            continuous_actions=self.continuous_actions,
            seed=self.seed,
            device=self.config.sampling_device,
            max_episodes=self.config.evaluation_episodes,
            env_type="train",
            adaptive_preference_sampler=self.adaptive_preference_sampler,
        )
        test_env = test_env_fun()
        
        n_eval = self.config.evaluation_episodes
        n_train = self.config.n_envs_per_worker(self.on_policy)
        train_worker_offset = n_eval  + 1

        if self.config.evaluation_episodes > 1 and test_env.batch_size == ():
            # The task env is not vectorized: emulate vectorized evaluation
            # through SerialEnv/ParallelEnv wrappers.
            test_env.close()
            print("--------------test_env successfully closed, start to initialize evaluation env---------------------------------")
            # eval_env_fun = self.task.get_env_fun(
            #     num_envs=1,
            #     continuous_actions=self.continuous_actions,
            #     seed=self.seed,
            #     device=self.config.sampling_device,
            ...
            env_class = SerialEnv if not self.config.parallel_evaluation else ParallelEnv
            # test_env = env_class(self.config.evaluation_episodes, eval_env_fun)
            test_env = env_class(n_eval, eval_env_fns)
            test_env.reset()
        ...
        env_func = self.task.get_env_fun(
                    worker_id=train_worker_offset,
                    num_envs=self.config.n_envs_per_worker(self.on_policy),
                    continuous_actions=self.continuous_actions,
                    seed=self.seed,
                    device=self.config.sampling_device,
                    max_episodes=self.config.n_envs_per_worker(self.on_policy),
                    env_type="train",
                    adaptive_preference_sampler=self.adaptive_preference_sampler,
                )
        ...
        # Initialize train env: decide vectorization from the training env itself,
        # not from test_env (which may be wrapped for parallel evaluation).
        train_probe_env = env_func()
        try:
            train_env_is_vectorized = train_probe_env.batch_size != ()
            print("train_env.batch_size: ", train_probe_env.batch_size)
        finally:
            train_probe_env.close()

        if not train_env_is_vectorized:
            ...
            env_fns = [
                self.task.get_env_fun(
                    worker_id=train_worker_offset + i,   # 8, 9, 10, ... 15
                    num_envs=1,
                    continuous_actions=self.continuous_actions,
                    seed=self.seed,
                    device=self.config.sampling_device,
                    max_episodes=n_train,
                    env_type="train",
                    adaptive_preference_sampler=self.adaptive_preference_sampler,
                ) for i in range(1, n_train + 1) ]

            if self.model_config.is_rnn:
                env_fns = [_add_rnn_transforms(env_fn, self.group_map, self.model_config) for env_fn in env_fns]
            self.env_func = lambda: TransformedEnv(env_class(n_train, env_fns),
                    transforms_training.clone(),)
        else:
    ...
    def _collection_loop(self):
        ...
        for _ in range(
            ...
            if not self.config.collect_with_grad:
                ...
                # print("[BATCH obs]", batch.get(("next","agents","observation")).shape)
                # print("[BATCH img]", batch.get(("next","agents","image")).shape)
            else:
    ...
    def _evaluation_loop(self):
        ...
        with set_exploration_type(
            ...
            if self.test_env.batch_size == ():
                ...
                rollouts = []
                for _ in range(self.config.evaluation_parallel_rollout_rounds):
                    inner_rollouts = self.test_env.rollout(
                        max_steps=self.max_steps,
                        policy=self.policy,
                        callback=callback if len(rollouts) == 0 else None,
                        auto_cast_to_device=True,
                        break_when_any_done=False,
                    )
                    rollouts.extend(list(inner_rollouts.unbind(0)))  # 关键：extend + unbind(0)
        ...
        eval_metric_sums = None 
        success_count = 0
        for rollout in rollouts:
            terminated = rollout["next", "terminated"].squeeze(-1)  # [T]
            truncated = rollout["next", "truncated"].squeeze(-1)    # [T]
            done = rollout["next", "done"].squeeze(-1)              # [T]
            manual_metrics = rollout["next", "manual_metrics"]
            done_steps = done.nonzero(as_tuple=True)[0]
            if done_steps.numel() > 0:
                end_idx = int(done_steps[0].item())
            else:
                end_idx = terminated.shape[0] - 1  # fallback (rare)

            is_success = bool(terminated[end_idx].item()) and (not bool(truncated[end_idx].item()))
            success_count += int(is_success)
      
            # SARWrapper emits final metrics and done in the same transition.
            last_valid_idx = (
                int(done_steps[0].item())
                if done_steps.numel() > 0
                else manual_metrics.shape[0] - 1
            )
            last_metrics = manual_metrics[last_valid_idx].reshape(-1)
            if eval_metric_sums is None:
                eval_metric_sums = [0.0] * last_metrics.shape[0]
            
            for i in range(len(eval_metric_sums)):
                eval_metric_sums[i] += last_metrics[i].item()
        
        # hv
        # hv, hv_points = compute_explore_path_hypervolume(rollouts, [0.0, -1500.0])
        # sampler_w = np.stack([r["agents", "preference"][0, 0].cpu().numpy() for r in rollouts])  # (N, 2)

        minmax_points = self.task.config.get("minmax_points",[[0.0, 0.0], [0.0, 0.0]],)

        if not self.task.config.get("condition_on_preference"):
            fixed_pref = self.task.config.get("fixed_preference", [1.0, 0.0])
            u_fixed = float(fixed_pref[0])
            u_arr = np.full(len(rollouts), u_fixed, dtype=np.float64)
            print(f"preference status:{self.task.config.get('condition_on_preference')} minmax_points: {minmax_points} u_arr: {u_arr}")
        else:
            u_arr = np.array([float(r["agents", "preference"][0, 0, 0].item()) for r in rollouts])
            print(f"preference status:{self.task.config.get('condition_on_preference')} minmax_points: {minmax_points} u_arr: {u_arr}")
        mo = compute_explore_path_mo_metrics(rollouts, minmax_points)
        
        self.logger.log(
            {
                "evaluation/mo/hypervolume": float(mo["hv"]),
                "evaluation/mo/hv_n_points": float(mo["points"].shape[0]),
                "evaluation/mo/cardinality": float(mo["cardinality"]),
                "evaluation/mo/sparsity": float(mo["sparsity"]),
                "evaluation/mo/n_episodes": float(mo["n_episodes"]),
                # "evaluation/mo/preference_u_mean": float(u_arr[0]),
            },
            step=self.n_iters_performed,
        )
        # Update shared preference after evaluation
        if self.adaptive_preference_sampler is not None:
            self.adaptive_preference_sampler.update(mo["points"])
        ...
        table_data = [
            [float(u_arr[i]), float(F[i, 0]), float(F[i, 1])]
            for i in range(len(F))
        ]
        
        all_table = Table(
            data=table_data,
            columns=["preference_u", "exploration_ratio", "neg_path_length"],
        )
        ...
        for logger in self.logger.loggers:
            if isinstance(logger, WandbLogger):
                logger.experiment.log({"evaluation/mo/scatter_chart": scatter_chart,
                                       "evaluation/mo/table": all_table}, step=self.n_iters_performed)
          

        # Logging Manual evaluation metrics
        win_rate = success_count / total_episodes
        eval_metric_avgs = [s / total_episodes for s in eval_metric_sums]
        
        evaluation_time = time.time() - evaluation_start
        self.logger.log(
            {"timers/evaluation_time": evaluation_time, 
             "evaluation/win_rate": win_rate}, step=self.n_iters_performed
        )
        # Logging Manual evaluation metrics
        dict_metrics = {
        ...
        # dict_metrics = {0: "CarSpeed"}

        for i, avg in enumerate(eval_metric_avgs):
            self.logger.log(
            {f"evaluation/{dict_metrics[i]}_avg": avg}, step=self.n_iters_performed)
    ...
    def _update_shared_preference_after_eval(self, mo: dict, rollouts) -> None:
        new_intervals = self._compute_preference_intervals_from_eval(mo, rollouts)
        if new_intervals is None:
            return

        self.adaptive_preference_sampler.update(new_intervals)

        # self.logger.log(
        #     {
        #         "pref_sampler/version": float(self.adaptive_preference_sampler.version.value),
        #         "pref_sampler/n_intervals": float(len(new_intervals)),
        #     },
        #     step=self.n_iters_performed,
        # )
